...
## 4. Cross-host install & validation — IN PROGRESS ← NEXT BUCKET

Install docs written for Claude Code, Claude Cowork, and GitHub Copilot CLI. Unblocked now that Epic 5 is merged. Alberto hands-on with the examples/field-tests/ photos (or fresh ones) per the checklist in specs/epic-4-cross-host-validation.md.

## 5. Consulting layout conventions — DONE (2026-07-10)

Bounded equidistant distribution, flush snap for header chips, canonical frame ratios (2/3+1/3, thirds, 25/50/25), contract-level `table` type expanded to lines + header boxes + borderless text (never the native PPT table object), consulting transcription rules in SKILL.md, tracked photo fixtures, Allium spec. PRs #5–#9 (each independently reviewed), issues #1–#4 closed. Self-checks: tests/run.py.

## 6. Publish v1 — PENDING

Blocked on Epic 4. Repo is public at github.com/albertojb/sketch-to-slide; v1 tag waits on validation.
