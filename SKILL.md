...
- Default to `rect`. Use `rounded_rect` ONLY when the corners are unmistakably drawn rounded — hand-drawn wobble is not rounding.
...
- Arrow direction follows the drawn arrowhead: `from` is the tail, `to` is the head. Arrowheads on both ends means `double_arrow`; no arrowhead means `line`. When in doubt, `line` — never add arrowheads the sketch doesn't show.
- A title written across the top of the sketch goes in the top-level `title` field, not in `elements`.
- A title band or label box on top of a bigger box or frame (a header chip) is its own `rect` with `"header": true` — whether drawn above the frame or as the frame's top compartment, either way. Don't fuss over its height or exact seat — the renderer seats it flush on the body box and renders it as a slim black band with white bold left-aligned text (consulting section-header style).
